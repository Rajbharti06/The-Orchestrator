...
export function startServer(port, orchestratorEvents) {
  ...
  // ── Spec-to-Product endpoint ─────────────────────────────────────────────

  app.post('/spec', async (req, res) => {
    try {
      const { parseSpec } = await import('./specParser.js');
      const input = req.body.spec || req.body.content || req.body.prompt || '';
      if (!input) return res.status(400).json({ error: 'spec field required' });
      const buildSpec = await parseSpec(input);
      if (req.body.dryRun) return res.json(buildSpec);
      // Enqueue build with parsed spec
      const job = jobQueue.enqueue(buildSpec.description || buildSpec.title, {
        spec: buildSpec,
        suggestedStack: buildSpec.suggestedStack,
        deploy: req.body.deploy || false,
      }, 0);
      res.json({ jobId: job.id, buildSpec });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
  ...
  app.post('/swarm', async (req, res) => {
    try {
      const { runSwarm, coordinateSwarms } = await import('./swarmCoordinator.js');
      const { swarms, task, mode = 'parallel' } = req.body;
      if (!task) return res.status(400).json({ error: 'task field required' });
      let result;
      if (Array.isArray(swarms) && swarms.length > 1) {
        result = await coordinateSwarms(swarms, task);
      } else {
        const swarmName = Array.isArray(swarms) ? swarms[0] : (swarms || 'review');
        result = await runSwarm(swarmName, task, { mode });
      }
      res.json(result);
    } catch (err) {
      res.status(500).json({ error: err.message });
  ...
  app.get('/swarm/status', async (_req, res) => {
    const { getSwarmStatus } = await import('./swarmCoordinator.js');
  ...
  app.get('/worktrees', async (_req, res) => {
    try {
      const { listWorktrees } = await import('./worktreeManager.js');
  ...
  app.delete('/worktrees/:agent', async (req, res) => {
    try {
      const { removeWorktree } = await import('./worktreeManager.js');
      removeWorktree(req.params.agent);
      res.json({ removed: req.params.agent });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
  ...
  app.post('/ci-fix', async (req, res) => {
    try {
      const { autofixCILoop } = await import('./ciLoop.js');
      const { files, plan, context, ciResult, maxAttempts = 3 } = req.body;
      if (!files || !plan) return res.status(400).json({ error: 'files and plan required' });
      const result = await autofixCILoop({ files, plan, sharedContext: context || {}, ciResult, maxAttempts });
      res.json(result);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.post('/rag', async (req, res) => {
    try {
      const { ragQuery } = await import('./ragEngine.js');
      const { query, topK = 5, namespace = 'all' } = req.body;
      if (!query) return res.status(400).json({ error: 'query field required' });
      const result = await ragQuery(query, { topK, namespace });
      res.json(result);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  // ── Continuity Memory endpoints ──────────────────────────────────────────

  app.get('/memory/episodes', async (req, res) => {
    const { getEpisodes } = await import('./continuityMemory.js');
    res.json(getEpisodes(req.query));
  });

  app.get('/memory/semantic', async (req, res) => {
    const { getSemanticPatterns } = await import('./continuityMemory.js');
    res.json(getSemanticPatterns(req.query.domain));
  });
